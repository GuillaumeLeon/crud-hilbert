...
@section('content')
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header">Edit transaction : {{ $transaction->title }}</div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-6">
                                    <input id="title" type="text"
                                           ...
                                           value="{{ old('title', $transaction->title) }}" required autocomplete="title"
                                           autofocus>
                                ...
                                <div class="col-md-6">
                                    <select class="form-control @error('status_id') is-invalid @enderror"
                                        ...
                                        @foreach(\App\Models\TransactionStatus::all() as $status)
                                            <option
                                                {{ (old('status') !== null && old('status') === $status->id) || $status->id === $transaction->status_id ? 'selected' : '' }} value="{{ $status->id }}">{{ $status->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
